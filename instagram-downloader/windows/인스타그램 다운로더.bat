...
# - 설치 불필요. 함께 제공된 .bat 파일을 더블클릭하면 실행됩니다.
...
try {
    ...
    function Unesc([string]$u) { return ($u -replace '\\u0026', '&' -replace '\\/', '/' -replace '&amp;', '&') }
    ...
    function Extract-Images([string]$html) {
        ...
        $add = {
            ...
            if (-not $seen.ContainsKey($u)) { $seen[$u] = $true; $out.Add($u) }
        }
        foreach ($m in [regex]::Matches($html, '"display_url":"(https:[^"\\]*(?:\\.[^"\\]*)*)"')) {
            & $add (Unesc $m.Groups[1].Value)
        }
        foreach ($m in [regex]::Matches($html, 'class="EmbeddedMediaImage"[^>]*src="([^"]+)"')) {
            & $add (DecHtml $m.Groups[1].Value)
        }
        $og = [regex]::Match($html, '<meta[^>]+property="og:image"[^>]+content="([^"]+)"')
        if ($og.Success) { & $add (DecHtml $og.Groups[1].Value) }
        return $out
    }
    function Extract-Videos([string]$html) {
        $out = New-Object System.Collections.Generic.List[string]
        foreach ($m in [regex]::Matches($html, '"video_url":"(https:[^"\\]*(?:\\.[^"\\]*)*)"')) {
            $out.Add((Unesc $m.Groups[1].Value))
        }
        $og = [regex]::Match($html, '<meta[^>]+property="og:video"[^>]+content="([^"]+)"')
        if ($og.Success) { $out.Add((DecHtml $og.Groups[1].Value)) }
        return $out
    }
    function Extract-Caption([string]$html) {
        # 1) 임베드 .Caption 블록
        $m = [regex]::Match($html, '(?s)<div class="Caption">(.*?)<div class="CaptionComments"')
        if (-not $m.Success) { $m = [regex]::Match($html, '(?s)<div class="Caption">(.*?)</div>') }
        if ($m.Success) {
            $c = $m.Groups[1].Value
        ...
        # 2) JSON edge_media_to_caption
        $jm = [regex]::Match($html, '"edge_media_to_caption":\{"edges":\[\{"node":\{"text":"((?:\\.|[^"\\])*)"')
        if (-not $jm.Success) { $jm = [regex]::Match($html, '"caption":\{[^}]*"text":"((?:\\.|[^"\\])*)"') }
        if ($jm.Success) {
            try { return ('"' + $jm.Groups[1].Value + '"' | ConvertFrom-Json) } catch {}
        }
        # 3) og:description
    ...
    # ---------------- 상태 ----------------
    $script:items = @()          # @{ Url; Type; Ext; Bytes }
    $script:shortcode = ""
    $script:caption = ""
    ...
    $btnGo.Add_Click({
        ...
        $embed = "https://www.instagram.com/p/$sc/embed/captioned/"
        ...
        if ([string]::IsNullOrWhiteSpace($all)) {
            Set-Status "콘텐츠를 불러오지 못했습니다. 인터넷 연결을 확인하고 잠시 뒤 다시 시도해 주세요." ([System.Drawing.Color]::FromArgb(224, 49, 49))
            $btnGo.Enabled = $true
            return
        }
        ...
        $list = @()
        foreach ($u in $imgs) { $list += @{ Url = $u; Type = "image" } }
        foreach ($u in $vids) { $list += @{ Url = $u; Type = "video" } }

        if ($list.Count -eq 0 -and [string]::IsNullOrWhiteSpace($cap)) {
            Set-Status "이미지·본문을 찾지 못했습니다. 비공개 게시물이거나 인스타그램이 로그인 화면을 반환했을 수 있습니다." ([System.Drawing.Color]::FromArgb(224, 49, 49))
            $btnGo.Enabled = $true
            return
        }
        ...
        foreach ($it in $list) {
            ...
            $item = @{ Url = $it.Url; Type = $it.Type; Ext = $ext; Bytes = $null }
            ...
            if ($it.Type -eq "image") {
                ...
                $pbLbl = New-Object System.Windows.Forms.Label
                $pbLbl.Text = "[ 동영상 ]"
                $pbLbl.TextAlign = "MiddleCenter"
                $pbLbl.Dock = "Fill"
                $pb.Controls.Add($pbLbl)
            }
            ...
            $btn.Text = $(if ($it.Type -eq "video") { "영상 저장" } else { "이미지 저장" })
            $btn.FlatStyle = "Flat"
        ...
        $cnt = $list.Count
        Set-Status ("완료: 미디어 " + $cnt + "개" + $(if ($cap) { " · 본문 확인됨" } else { "" })) ([System.Drawing.Color]::FromArgb(47, 158, 68))
        $btnGo.Enabled = $true
    })
