...
이미 만들어 둔 이미지(예: 그록으로 뽑은 그림)를 그대로 쓸 수 있습니다.

- 결과 화면의 각 이미지 카드에서 **`📁 내 이미지`** 로 파일을 올리면 그 슬롯에 저장됩니다.
- **`🖼️ 이미지 생성`** 은 이미 있는(올린/생성된) 이미지는 **건너뛰고** 비어 있는 것만 만듭니다 → 중복 비용 없음.
- **인트로 영상**도 그 슬롯의 이미지를 재료로 사용합니다(image-to-video).
- 마음에 안 드는 슬롯만 **`↻ 다시`** 로 한 장씩 다시 생성할 수 있어요.
- (수동) `output/<slug>/images/` 에 `img-01.png`, `img-02.png` … 형식으로 직접 넣어도 됩니다.
