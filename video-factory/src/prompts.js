...
- 대본에 채널명·자기소개·인사말을 절대 넣지 마라. "안녕하세요 ○○입니다", "저는/여기는 ○○입니다" 같은 멘트 금지. 첫 문장은 곧바로 후킹으로 시작한다.
...
- 각 prompt 에는 반드시 세 가지를 구체적으로 넣어라: (a) 배경/장소 — 현실에서 볼 수 있는 실제 장소를 '장면마다 다르게'(카페, 집 거실, 거리, 공원, 사무실, 식당, 지하철 등), (b) 배경 색감·분위기 — 실제 장소처럼 자연스럽고 '장면마다 다양하게'. 절대 한 가지 색조(예: 온통 분홍/파스텔)로 통일하지 말 것, (c) 인물의 구체적 행동·표정·몸짓(그 챕터 내용과 일치).
- [구성 다양화] 이미지가 지루하지 않게 컷마다 구도·앵글·인물 구성을 다르게 하라: 혼자(남 또는 여)·둘이 함께·클로즈업·와이드·뒷모습·손/사물 클로즈업·상황(배경) 컷 등을 골고루 섞어라. 매 컷 같은 구도(정면 한 명) 반복 금지.
- [성별] 한 성별만 계속 나오지 않게 하라. 대본상 상호작용/관계 장면은 남녀를 함께, 혼자 있는 장면은 혼자 그려라. 대본 내용에 맞게 자연스럽게 배분.
